...
def symbols_are_equal(symbol1, symbol2):
    ...
    props1 = [item for item in symbol1 if isinstance(item, list) and item[0] == 'property']
    props2 = [item for item in symbol2 if isinstance(item, list) and item[0] == 'property']
    ...
    def compare_properties(prop1, prop2):
        ...
        effects1 = next((x for x in prop1 if isinstance(x, list) and x[0] == 'effects'), None)
        effects2 = next((x for x in prop2 if isinstance(x, list) and x[0] == 'effects'), None)
    ...
    units1 = [item for item in symbol1 if isinstance(item, list) and item[0] == 'symbol']
    units2 = [item for item in symbol2 if isinstance(item, list) and item[0] == 'symbol']
    ...
    def compare_units(unit1, unit2):
        ...
        geom1 = [item for item in unit1 if isinstance(item, list) and item[0] not in ('pin', 'text')]
        geom2 = [item for item in unit2 if isinstance(item, list) and item[0] not in ('pin', 'text')]
        ...
        def compare_pins(pin1, pin2):
            ...
            num1 = next((x[1] for x in pin1 if isinstance(x, list) and x[0] == 'number'), None)
            num2 = next((x[1] for x in pin2 if isinstance(x, list) and x[0] == 'number'), None)
            if num1 != num2:
                return False
                
            name1 = next((x[1] for x in pin1 if isinstance(x, list) and x[0] == 'name'), None)
            name2 = next((x[1] for x in pin2 if isinstance(x, list) and x[0] == 'name'), None)
            if name1 != name2:
                return False
            ...
            # Compare pin length
            len1 = next((x[1] for x in pin1 if isinstance(x, list) and x[0] == 'length'), None)
            len2 = next((x[1] for x in pin2 if isinstance(x, list) and x[0] == 'length'), None)
            if len1 != len2:
                return False
                
            # Compare hide attribute if present
            hide1 = next((x for x in pin1 if isinstance(x, list) and x[0] == 'hide'), None)
            hide2 = next((x for x in pin2 if isinstance(x, list) and x[0] == 'hide'), None)
        ...
        for pin in pins1:
            num = next((x[1] for x in pin if isinstance(x, list) and x[0] == 'number'), None)
            if num:
